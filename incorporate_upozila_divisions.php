...
foreach ($upazilas_json['upazilas'] as $upazila) {
    $district_id = $upazila['district_id'];
    if (!isset($upazilas_by_district[$district_id])) {
        $upazilas_by_district[$district_id] = [];
    }
    $upazilas_by_district[$district_id][] = $upazila;
}

// Step 2: Add upazilas to the respective districts in main_json
foreach ($main_json['divisions'] as &$division) {
    foreach ($division['districts'] as &$district) {
        $district_id = $district['id'];
        // Check if upazilas exist for this district_id
        if (isset($upazilas_by_district[$district_id])) {
            $district['upozilla'] = $upazilas_by_district[$district_id];
        } else {
            $district['upozilla'] = []; // Set an empty array if no upazilas found
        }
    }
}

header('Content-Type: application/json');
echo json_encode($main_json, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
?>
