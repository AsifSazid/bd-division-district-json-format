...
// Step 1: Collect upazilas by district_id
const upazilasByDistrict = upazilasJson.upazilas.reduce((acc, upazila) => {
    const { district_id } = upazila;
    if (!acc[district_id]) {
        acc[district_id] = [];
    }
    acc[district_id].push(upazila);
    return acc;
}, {});

// Step 2: Add upazilas to the respective districts in mainJson
mainJson.divisions.forEach(division => {
    division.districts.forEach(district => {
        const districtId = district.id;
        district.upozila = upazilasByDistrict[districtId] || [];
    });
});

console.log(JSON.stringify(mainJson, null, 4));
